...
(function () {
  ...
  function renderHome() {
    ...
    const modeSel = h('select', { class: 'cfg-select' },
      [['word', '單字'], ['phrase', '片語']].map(([v, t]) => h('option', { value: v }, t)));
    ...
    root.append(h('div', { class: 'part-cards', style: 'grid-template-columns:1fr' },
      h('div', { class: 'part-card' },
        ...
        h('div', { class: 'p-stats' },
          '最佳分數:單字 ' + (best['word'] || 0) + ' · 片語 ' + (best['phrase'] || 0)),
  ...
  function startGame(mode, levelName, speedKey) {
    const pool = mode === 'word' ? buildWordPool(levelName) : buildPhrasePool();
    if (pool.length < 8) {
      renderHome();
      return;
    }
    ...
    root.append(
      h('div', { class: 'drill-top' },
        h('h1', null, '掉落消除 ', h('span', { style: 'font-size:13.5px;color:var(--ink-light);font-weight:400' }, mode === 'word' ? '單字' + (levelName !== '全部' ? ' · ' + levelName : '') : '片語')),
