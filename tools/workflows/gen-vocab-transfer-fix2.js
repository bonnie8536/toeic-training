export const meta = {
  name: 'gen-vocab-transfer-fix2',
  description: '補寫最後 2 篇單字移轉考題(art-9b/12b):允許捨棄無法錨定的近義字,盲解+歧義+規範三線驗證',
  phases: [
    { title: 'Generate', detail: '2 篇各自重寫' },
...
const IDS = ['art-9b', 'art-12b']
const PRIOR = {
  'art-9b': '前三版都敗在同一點:boss 與 manager 兩格可互換、package 與 product 可互換。這次【直接不要用 boss,也不要用 package】(只用 manager 與 product;其餘字照 vocab),挖空總數維持 ≥8。manager 要接部門或場所(the store manager / the sales manager);product 用「the product itself / a new product line」這類指商品本身的搭配。情境要與原文(新人走錯會議室)完全不同。',
  'art-12b': '前三版都敗在 crop/harvest 可互換、stock 可當動詞、customers/subscribers 可互換。這次【不要用 crop,不要用 stock,不要用 subscribers】(保留 harvest、customers;其餘字照 vocab,總數 ≥8 即可)。harvest 用名詞且加季節或動作錨定(the autumn harvest was small / after the harvest);customers 用「customers who walk in / regular customers」。passageZh 逐句忠實,不增譯任何原文沒有的地名或修飾。情境要與原文(醜蔬菜/農民/超市退貨)完全不同。',
}
...
每篇規則:
1. 從該篇 vocab 挑單字:一般 8-12 個;【本輪特別允許】捨棄指定的近義字(見該篇指示),挖空總數 ≥8 即可。挑內容承載力高的(名/動/形兼顧)。
2. 寫一篇全新短文,情境必須與原文(見 title/titleZh/summary)完全不同:換產業、換場景、換人物、換劇情骨幹。單字語意必須用 hint/zh 教過的那個義項。
...
5. 【反歧義,最重要】拿掉所有標記後,每格必須能由上下文唯一決定:同詞性的字兩兩檢查——互換後必須在語意上明顯錯誤;用搭配詞、指涉對象、前後因果把每格釘死。能當名詞也能當動詞的字,句型要讓詞性只有一種可能。想像學生看著字卡列表逐格推理,不能有兩格可對調。
...
log('初生成通過結構檢查 ' + Object.keys(pool).length + '/2')
...
log('首輪通過 ' + Object.keys(pool).length + '/2,待重生成 ' + Object.keys(failed).length)
...
for (let round = 1; round <= 3 && Object.keys(failed).length; round++) {
  ...
  const regenned = await parallel(targets.map(([id, reasons]) => () =>
    agent(RULES + '\n\n你只負責重寫這一篇:' + id + '\n' + PRIOR[id] + '\n最新一版仍未通過,原因:\n- ' + reasons.join('\n- ') + '\n請針對原因徹底改寫(歧義問題=加強每格的搭配詞/指涉錨定,或再捨棄難以錨定的字,總數 ≥8)。回傳單篇物件。', { label: 'regen' + round + ':' + id, phase: 'Regen', schema: ONE_SCHEMA, effort: 'high' })))
  const reverify = []
  ...
  log('重生成第 ' + round + ' 輪後通過 ' + Object.keys(pool).length + '/2,仍失敗 ' + Object.keys(failed).length)
}
